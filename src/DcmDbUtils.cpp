...
#define SELECT_MEDIA_UUID_BY_FILE_PATH_FROM_DB "SELECT media_uuid FROM media WHERE path = '%q';"
...
int DcmDbUtils::_dcm_svc_db_get_media_uuid_by_path(const char *file_path, char **media_uuid)
{
	int ret = MS_MEDIA_ERR_NONE;
	char *query_string = NULL;
	sqlite3_stmt *sql_stmt = NULL;
	...
	query_string = sqlite3_mprintf(SELECT_MEDIA_UUID_BY_FILE_PATH_FROM_DB, file_path);
	...
	if (DCM_STRING_VALID((const char *)sqlite3_column_text(sql_stmt, 0)))
		*media_uuid = strdup((const char *)sqlite3_column_text(sql_stmt, 0));
	else {
		dcm_error("Failed to get media_uuid from DB");
...
int DcmDbUtils::_dcm_svc_db_update_color_to_db(DcmColorItem color)
{
	...
	query_string = sqlite3_mprintf(UPDATE_COLOR_ITEM_TO_DB, (int)(color.rgb_r[0]), (int)(color.rgb_g[0]), (int)(color.rgb_b[0]), color.media_uuid, color.storage_uuid);
	dcm_debug("query is %s", query_string);
...
int DcmDbUtils::_dcm_svc_db_send_noti(DcmFaceItem *face, DcmFaceItemUpdateItem update_item, DcmFaceItemUpdateType update_type)
{
	...
	if (face == NULL)
	{
		...
		//ret = media_face_db_update_send(face->contact_id, face->cluster_id, (media_face_item_type_e)update_item, (media_face_item_update_type_e)update_type, face->media_id, face->face_uuid);
	}
...
int DcmDbUtils::_dcm_svc_db_send_noti(int contact_id, int cluster_id, DcmFaceItemUpdateItem update_item, DcmFaceItemUpdateType update_type, char* media_id, char* uuid)
{
	int ret = MS_MEDIA_ERR_NONE;

	dcm_debug_fenter();

	//ret = media_face_db_update_send(contact_id, cluster_id, (media_face_item_type_e)update_item, (media_face_item_update_type_e)update_type, media_id, uuid);
	if (ret != MS_MEDIA_ERR_NONE) {
		dcm_error("Failed to update face! err: %s", ret);
	}
