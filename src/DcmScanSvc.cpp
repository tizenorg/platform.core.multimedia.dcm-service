...
int DcmScanSvc::ScanSingleItem(const char *file_path)
{
	...
	DCM_CHECK_VAL(scan_thread_main_context, DCM_ERROR_INVALID_PARAMETER);
	DCM_CHECK_VAL(file_path, DCM_ERROR_INVALID_PARAMETER);

	DcmScanItem *scan_item = (DcmScanItem *) g_malloc0(sizeof(DcmScanItem));
	DCM_CHECK_VAL(scan_item, DCM_ERROR_OUT_OF_MEMORY);

	scan_item->file_path = strdup(file_path);
	if (scan_item->file_path == NULL) {
		dcm_error("strdup file_path failed. Out of memory");
	...
	scan_single_item_list = g_list_append(scan_single_item_list, scan_item);

	/* DCM scan started */
	if (scan_single_item_list != NULL && scan_single_curr_index < g_list_length(scan_single_item_list)) {
