...
int DcmColorUtils::runColorExtractProcess(DcmScanItem *scan_item, DcmImageInfo *image_info)
{
	...
	ret = image_util_extract_color_from_memory(image_info->pixel, image_info->buffer_width, image_info->buffer_height, IMAGE_UTIL_COLORSPACE_RGB888, colorItem.rgb_r, colorItem.rgb_g, colorItem.rgb_b);
	if (ret != IMAGE_UTIL_ERROR_NONE) {
		dcm_error("image_util_extract_color_from_memory err= %d", ret);
		return DCM_ERROR_IMAGE_UTIL_FAILED;
	}

	dcm_debug("image_util_extract_color_from_memory result r:%02x, g:%02x, b:%02x", colorItem.rgb_r[0], colorItem.rgb_g[0], colorItem.rgb_b[0]);
